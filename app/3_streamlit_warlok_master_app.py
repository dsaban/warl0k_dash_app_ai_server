# streamlit_warlok_master_app.py
# Run:
#   streamlit run streamlit_warlok_master_app.py
#
# MASTER APP (Upgraded):
# Tab 1: P2P Protocol Simulator
#   - True P2P (no client/server) duplex wire
#   - TLS-like handshake + record layer (with delimiter-safe base64 framing)
#   - IAM + PAM + WARL0K START anchor
#   - 48-step continuity windows (dt/counter/step/window + mac_chain)
#   - Attack mutations + gate-by-gate trace + progress + timelines + scoreboard
#
# Tab 2: Nano-AI MLEI (NumPy-only)
#   - Dataset generation (pos + multiple negative attacks)
#   - Phase1 training (MS reconstruction + optional next-token scaffold)
#   - Phase2 training (ID head + Window head + validity head)
#   - Interactive verification for legit + attack scenarios
#   - Gate table + charts + attention weights
#
# Notes:
# - Crypto and TLS are TOY for simulation; the integrity logic and gating patterns are what’s being demonstrated.
# - All-in-one file for easy demo.
...
# ---- Toy stream cipher for record encryption (not real TLS)
...
class P2PTLS:
    ...
    Symmetric handshake:
    - HS1 exchange nonces
    - derive PRK with canonical nonce ordering (by peer IDs)
    - HS2 FIN exchange
    Record layer uses delimiter-safe base64 framing.
    """
    ...
    def handshake_step2_recv_and_derive(self) -> bool:
        ...
        # canonical ordering => both sides derive identical keys
...
def simulate_protocol_run(
    ...
    # Symmetric handshake (no client/server)
    ...
    _claimB = iam_authenticate(peerB_id, "peerB_op_creds", postureB)
    ...
    # Train profile on first <=48 steps
    wsA_train = init_ws()
...
# ============================================================
# TAB 2 — NumPy Nano-AI MLEI (GRU+Attention)
# ============================================================

# ---- Constants (kept small-ish; UI can scale epochs)
VOCAB_SIZE = 16
MS_DIM     = 8
SEQ_LEN    = 20
...
def verify_chain_ai(p, tokens, meas, claimed_id, expected_w, true_ms=None):
    ...
    if true_ms is not None:
        out["l2_ms"] = float(np.linalg.norm(ms_hat - true_ms))
    return out

def attack_shuffled_ai(tokens, meas, seed=0):
    rng = np.random.RandomState(seed)
    idxs = np.arange(len(tokens))
    rng.shuffle(idxs)
    return tokens[idxs], meas[idxs]

def attack_truncated_ai(tokens, meas):
    Ltr = len(tokens)//2
...
st.set_page_config(page_title="WARL0K Master: P2P + Nano-AI MLEI", layout="wide")
st.title("WARL0K Master Demo — P2P Continuity + Nano-AI MLEI")
st.caption("Two tabs: (1) Protocol/Continuity Proof chain (TLS+START+mac_chain) (2) NumPy Nano-AI MLEI (GRU+Attention).")

tab1, tab2 = st.tabs(["1) P2P Protocol + Continuity Proof", "2) Nano-AI MLEI (NumPy GRU+Attention)"])
...
with tab1:
    with st.sidebar:
        ...
        st.divider()
        st.subheader("Nano profile (Tab1) sensitivity")
    ...
    if not run:
        ...
        meta = simulate_protocol_run(steps, attack_mode, sleep_ms, slack_dt, slack_meas, op_pattern)
        if not meta.get("ok"):
...
"""
                ...
                with s3:
                    st.markdown("### Learned profile gates (Tab1 nano profile)")
                    g1, g2 = st.columns(2)
                    with g1:
            ...
            if show_scoreboard:
                ...
                with left:
                    ...
                    st.caption("Deterministic = session/window/step/counter/MAC. Nano = dt/op/meas profile.")
                with right:
            ...
            if drop_rows:
                st.warning("First DROP row (details)")
...
with tab2:
    with st.sidebar:
        ...
        seed = st.number_input("Random seed", min_value=0, max_value=10_000, value=0, step=1, key="ai_seed")
    ...
    X_ALL, M_ALL, TOK_ALL, Y_MS_ALL, Y_CLS_ALL, TRUE_ID, TRUE_W, CLAIM_ID, EXPECT_W = build_dataset(seed, pilot_amp)
    N = X_ALL.shape[0]
    ...
    if run_train:
        ...
        p = init_model(X_ALL.shape[2], seed=seed)
    ...
    with left:
        ...
        st.markdown(
            """
**Phase1 (Reconstruction):** GRU+Attention → MS reconstruction (positives), optional next-token scaffold early.
**Phase2 (MLEI heads):** Identity head + Window head + Validity head (uses claim + pid + pw).
**Decision:** OK only if all gates pass: `p_valid`, id match, window match, and confidence (`pid`,`pw`).
"""
        )
    with right:
    ...
    st.divider()
    st.markdown("## Verification + Attacks (AI)")
    if not st.session_state.ai_trained:
        st.info("Train Tab 2 model in the sidebar to enable verification.")
    else:
        p = st.session_state.ai_p
        MS_all, A_base = init_world(seed)
        ...
        if scenario == "ATTACK: SHUFFLED":
            toks2, meas2 = attack_shuffled_ai(toks, meas, seed=int(attack_seed))
        elif scenario == "ATTACK: TRUNCATED":
            toks2, meas2 = attack_truncated_ai(toks, meas)
        ...
        r = verify_chain_ai(p, toks2, meas2, claimed_id=claimed_id, expected_w=expected_w, true_ms=ms_true)

        cA, cB, cC, cD = st.columns([1.1, 1.1, 1.1, 1.7])
        with cA: st.metric("p_valid", f"{r['p_valid']:.4f}")
        with cB: st.metric("pid (claim prob)", f"{r['pid']:.4f}")
        with cC: st.metric("pw (expected window prob)", f"{r['pw']:.4f}")
        with cD: st.metric("Decision", "OK ✅" if r["ok"] else "REJECT ❌")

        st.markdown("### Gate-by-gate validation")
        gate_rows = [{"gate": k, "pass": bool(v)} for k, v in r["checks"].items()]
        gate_rows.append({"gate": f"l2(MS_hat, MS_true) (info) = {r.get('l2_ms', 0.0):.4f}", "pass": True})
        ...
        with m3:
            st.write("Model preds:", f"id_pred={r['id_pred']}, w_pred={r['w_pred']}")
