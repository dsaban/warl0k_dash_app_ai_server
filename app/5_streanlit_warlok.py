...
def attack_timewarp(window: List[ChainMsg], new_dt: int = 999999) -> List[ChainMsg]:
    # FIX: original code mutated dt_ms but kept the original mac_chain, so the
    # MAC gate fired first (hiding the dt anomaly gate). We intentionally DO NOT
    # recompute the mac_chain so that the dt anomaly fires as the first gate in
    # verify_msg (it is checked before mac). This is the correct semantic: the
    # attacker forges the timing field but cannot forge the MAC.
    w = []
    for i, m in enumerate(window):
        if i == 7:
            mm = ChainMsg(**m.__dict__)
            mm.dt_ms = new_dt
            # mac_chain intentionally left as original so verify catches dt first
            w.append(mm)
        else:
            w.append(m)
    return w
...
def attack_splice(window: List[ChainMsg]) -> List[ChainMsg]:
    # op_code is checked (allowlist gate) BEFORE mac_chain in verify_msg,
    # so the op allowlist gate fires first — correct behaviour.
    w = []
    for i, m in enumerate(window):
        if i == 12:
            mm = ChainMsg(**m.__dict__)
...
# ============================================================
# TAB 4 — RNN Attack Predictor (GRU multiclass classifier)
# ============================================================
# Design rationale:
#  - The existing Tab-2 model is a BINARY verifier: legit vs not.
#  - This NEW model is a 6-class ATTACK CLASSIFIER:
#      0=none, 1=reorder, 2=drop, 3=replay, 4=timewarp, 5=splice
#  - Input features per time-step: [meas, dt_norm, step_norm, ctr_norm,
#    is_accept (0/1), op_onehot(READ/WRITE/CONTROL)]  → 9-dim
#  - Architecture: single-layer GRU → mean-pool → softmax head
#  - Trained on synthetic traces generated by simulate_protocol_run
#    (one trace per attack type, repeated with random seeds).
# ============================================================

ATK_LABELS   = ["none", "reorder", "drop", "replay", "timewarp", "splice"]
ATK_N        = len(ATK_LABELS)
PRED_HDIM    = 48
PRED_IN_DIM  = 9   # see feature builder below
PRED_LR      = 0.01
PRED_EPOCHS  = 80
PRED_SEQ     = 48  # fixed sequence length for predictor


def _atk_features(trace: List[Dict[str, Any]], max_dt: float = 200.0) -> np.ndarray:
    """Build (PRED_SEQ, PRED_IN_DIM) feature matrix from a trace list.
    Features: [meas, dt_norm, step_norm, ctr_norm, is_accept, op_READ, op_WRITE, op_CONTROL, pos_norm]
    FIX: normalise dt_ms by a fixed cap (not per-trace max) so the model
    sees consistent scaling across attacks — timewarp shows as dt_norm >> 1.
    """
    op_map = {"READ": 0, "WRITE": 1, "CONTROL": 2}
    T = min(len(trace), PRED_SEQ)
    X = np.zeros((PRED_SEQ, PRED_IN_DIM), dtype=np.float32)
    for t in range(T):
        r = trace[t]
        dt_norm   = float(r["dt_ms"]) / max(max_dt, 1.0)
        step_norm = float(r["step"]) / (WINDOW_SIZE - 1)
        ctr_norm  = float(r["ctr"]) / max(PRED_SEQ, 1.0)
        is_acc    = 1.0 if r["decision"] == "ACCEPT" else 0.0
        pos_norm  = float(t) / max(PRED_SEQ - 1, 1.0)
        op_idx    = op_map.get(r["op"], 1)
        # lay out as 9 features
        X[t, 0] = float(r["meas"])
        X[t, 1] = dt_norm
        X[t, 2] = step_norm
        X[t, 3] = ctr_norm
        X[t, 4] = is_acc
        X[t, 5 + op_idx] = 1.0   # one-hot op (indices 5,6,7)
        X[t, 8] = pos_norm
    return X


def _gru_forward_single(X: np.ndarray, p: dict) -> np.ndarray:
    """(T, D) → (T, H) hidden states, single-sample GRU."""
    T, _ = X.shape
    H = p["pH"].shape[0]  # hidden dim
    h = np.zeros(H, dtype=np.float32)
    hs = np.zeros((T, H), dtype=np.float32)
    for t in range(T):
        x = X[t]
        z = sigmoid_np(x @ p["pWz"].T + h @ p["pUz"].T + p["pbz"])
        r = sigmoid_np(x @ p["pWr"].T + h @ p["pUr"].T + p["pbr"])
        g = np.tanh(x   @ p["pWh"].T + (r*h) @ p["pUh"].T + p["pbh"])
        h = (1-z)*h + z*g
        hs[t] = h
    return hs


def _gru_forward_batch_pred(Xb: np.ndarray, p: dict) -> np.ndarray:
    """(B, T, D) → (B, H) mean-pooled context."""
    B, T, _ = Xb.shape
    H = p["pH"].shape[0]
    h = np.zeros((B, H), dtype=np.float32)
    hs_sum = np.zeros((B, H), dtype=np.float32)
    for t in range(T):
        x = Xb[:, t, :]
        z = sigmoid_np(x @ p["pWz"].T + h @ p["pUz"].T + p["pbz"])
        r = sigmoid_np(x @ p["pWr"].T + h @ p["pUr"].T + p["pbr"])
        ...
        h = (1-z)*h + z*g
        hs_sum += h
    return hs_sum / T   # mean pool


def init_pred_model(seed: int = 42) -> dict:
    rng = np.random.RandomState(seed)
    s = 0.08
    D, H, C = PRED_IN_DIM, PRED_HDIM, ATK_N
    p = {}
    p["pH"]   = np.zeros((H,), dtype=np.float32)  # placeholder for hidden dim
    for gate in ("z", "r", "h"):
        p[f"pW{gate}"] = (rng.randn(H, D) * s).astype(np.float32)
        p[f"pU{gate}"] = (rng.randn(H, H) * s).astype(np.float32)
        p[f"pb{gate}"] = np.zeros(H, dtype=np.float32)
    p["pWc"] = (rng.randn(C, H) * s).astype(np.float32)
    p["pbc"] = np.zeros(C, dtype=np.float32)
    return p


def _pred_grad_step(p: dict, Xb: np.ndarray, yb: np.ndarray,
                    opt_state: dict, lr: float) -> float:
    """
    One SGD step with Adam for the predictor.
    Uses numerical-gradient-free manual backprop through mean-pool GRU.
    FIX: use a dedicated Adam state dict rather than sharing with the main model.
    """
    B, T, D = Xb.shape
    H = PRED_HDIM

    # ---- forward ----
    ctx = _gru_forward_batch_pred(Xb, p)          # (B, H)
    logits = ctx @ p["pWc"].T + p["pbc"]          # (B, C)
    probs  = softmax_np(logits, axis=-1)           # (B, C)
    ...
    grads = {}
    grads["pWc"] = dlog.T @ ctx
    grads["pbc"] = dlog.sum(axis=0)
    dctx = dlog @ p["pWc"]    # (B, H)

    # ---- backward through GRU (BPTT via mean-pool) ----
    # dctx = dL/d(mean_h) → dL/d(h_t) = dctx / T for each t
    dh_from_ctx = dctx / T   # (B, H)

    # Re-run forward to collect cache
    ...
    hprev_arr = np.zeros((B, T, H), dtype=np.float32)
    ...
    for gk in ("pWz","pUz","pbz","pWr","pUr","pbr","pWh","pUh","pbh"):
        grads[gk] = np.zeros_like(p[gk])
    ...
    for t in reversed(range(T)):
        ...
        dag = dg * (1 - g*g)
        grads["pWh"] += dag.T @ x
        grads["pUh"] += dag.T @ (r*hp)
        grads["pbh"] += dag.sum(axis=0)
        dhp          += (dag @ p["pUh"]) * r
        dr             = (dag @ p["pUh"]) * hp
        ...
        grads["pWr"] += dar.T @ x
        grads["pUr"] += dar.T @ hp
        grads["pbr"] += dar.sum(axis=0)
        dhp          += dar @ p["pUr"]

        daz = dz * z * (1 - z)
        grads["pWz"] += daz.T @ x
        grads["pUz"] += daz.T @ hp
        grads["pbz"] += daz.sum(axis=0)
        dhp          += daz @ p["pUz"]

        dh_next = dhp

    # ---- Adam update (dedicated state) ----
    opt_state["t"] += 1
    b1, b2, eps = 0.9, 0.999, 1e-8
    t_step = opt_state["t"]
    for k, g_val in grads.items():
        opt_state["m"][k] = b1*opt_state["m"][k] + (1-b1)*g_val
        opt_state["v"][k] = b2*opt_state["v"][k] + (1-b2)*(g_val*g_val)
        mh = opt_state["m"][k] / (1 - b1**t_step)
        vh = opt_state["v"][k] / (1 - b2**t_step)
...
def build_attack_dataset(
    ...
    """
    Generate (X, y) where X is (N, PRED_SEQ, PRED_IN_DIM) and y is (N,) int labels.
    FIX: use a fresh random seed per sample so the model sees varied timing,
    avoiding it memorising a single trace fingerprint per class.
    """
    Xs, ys = [], []
    for cls_idx, atk in enumerate(ATK_LABELS):
        for i in range(n_per_class):
            rng_seed = seed * 1000 + cls_idx * 100 + i
            np.random.seed(rng_seed)
            # vary sleep to make dt distribution non-trivial
            sleep_v = int(np.random.randint(0, 4))
            try:
                meta = simulate_protocol_run(
                    steps=steps,
                    attack_mode=atk,
                    sleep_ms=sleep_v,
                    slack_dt=10,
                ...
                if not meta.get("ok") or len(meta["trace"]) == 0:
                    continue
                X = _atk_features(meta["trace"])
                Xs.append(X)
                ys.append(cls_idx)
            except Exception:
                continue
...
def train_pred_model(
    ...
    """Train the RNN attack predictor and return (params, loss_curve)."""
    X_all, y_all = build_attack_dataset(n_per_class=n_per_class, seed=seed)
    N = X_all.shape[0]

    p = init_pred_model(seed=seed)
    opt_state = {
        "t": 0,
    ...
    for ep in range(1, epochs + 1):
        ...
        for s in range(0, N, batch_size):
            ...
            if len(b) < 2:
                continue
            loss = _pred_grad_step(p, X_all[b], y_all[b], opt_state, lr)
            ep_loss += loss
            n_batches += 1
        avg = ep_loss / max(n_batches, 1)
        losses.append(avg)
        if log_cb and (ep == 1 or ep % max(1, epochs // 8) == 0):
            log_cb(f"[PredRNN] epoch {ep}/{epochs}  loss={avg:.4f}")

    return p, losses


def predict_attack(p_pred: dict, trace: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Run the trained RNN predictor on a single trace."""
    X = _atk_features(trace)[None, :, :]   # (1, T, D)
    ctx = _gru_forward_batch_pred(X, p_pred)
    logits = (ctx @ p_pred["pWc"].T + p_pred["pbc"])[0]
    probs  = softmax1d_np(logits)
    pred_idx = int(np.argmax(probs))
    return {
        "pred_label": ATK_LABELS[pred_idx],
        "pred_idx":   pred_idx,
...
def run_all_attacks_batch(
    steps: int = 48,
    sleep_ms: int = 0,
    slack_dt: int = 10,
    slack_meas: float = 0.02,
    op_pattern: str = "rw",
) -> List[Dict[str, Any]]:
    """
    Run all 6 attack scenarios and return results list.
    FIX: each scenario gets a fresh simulate_protocol_run call (independent
    session/chain-key) so attacks don't bleed state into each other.
    """
    results = []
    for atk in ATK_LABELS:
        meta = simulate_protocol_run(
            ...
            sleep_ms=sleep_ms,
            slack_dt=slack_dt,
            slack_meas=slack_meas,
            op_pattern=op_pattern,
            forensic_continue=True,
        )
        results.append({"attack": atk, "meta": meta})
    return results


def simulate_manual_attack(
    steps: int,
    edits: List[Dict[str, Any]],   # list of {msg_idx, field, value}
    sleep_ms: int = 0,
    slack_dt: int = 10,
    slack_meas: float = 0.02,
    op_pattern: str = "rw",
    forensic_continue: bool = True,
    target: str = "pump-controller",
    actions: List[str] = None,
) -> Dict[str, Any]:
    """
    Run a protocol simulation where the user manually specifies which message
    indices to tamper with and which fields to change.

    edits: list of dicts, each with:
        msg_idx  — 0-based index of the message to tamper
        field    — one of: dt_ms | op_code | step_idx | global_counter |
                           window_id | os_meas | session_id
        value    — new value (string; cast to correct type internally)

    Multiple edits can target the same message (all applied before sending).
    mac_chain is intentionally NOT recomputed so the MAC gate can fire,
    exactly like the built-in named attacks.
    """
    if actions is None:
        actions = ["READ", "WRITE"]
    ...
    tlsA.handshake_step1_send(); tlsB.handshake_step1_send()
    for _ in range(10):
        if not tlsA.keys: tlsA.handshake_step2_recv_and_derive()
        if not tlsB.keys: tlsB.handshake_step2_recv_and_derive()
        if tlsA.keys and tlsB.keys: break
    ...
    tlsA.handshake_step3_send_fin(); tlsB.handshake_step3_send_fin()
    okA = okB = False
    for _ in range(10):
        okA = okA or tlsA.handshake_step4_recv_verify()
        okB = okB or tlsB.handshake_step4_recv_verify()
    ...
    train_window = []
    for i in range(train_steps):
        op = ("READ" if i % 3 else "WRITE") if op_pattern == "rw" else ("READ" if op_pattern == "read" else "WRITE")
        train_window.append(build_chain_msg(chain_key, grant, wsA_train, op, f"op{i}".encode(), sleep_ms=sleep_ms))
    bundle = train_profile("peerA", grant, train_window, slack_dt=slack_dt, slack_meas=slack_meas)
    ...
    for i in range(steps):
        op = ("READ" if i % 3 else "WRITE") if op_pattern == "rw" else ("READ" if op_pattern == "read" else "WRITE")
        send_window.append(build_chain_msg(chain_key, grant, wsA_send, op, f"op{i}".encode(), sleep_ms=sleep_ms))

    # Build edit index: msg_idx → list of (field, value)
    edit_map: Dict[int, List[Tuple[str, Any]]] = {}
    INT_FIELDS   = {"dt_ms", "step_idx", "global_counter", "window_id"}
    FLOAT_FIELDS = {"os_meas"}
    STR_FIELDS   = {"op_code", "session_id"}
    for e in edits:
        idx   = int(e["msg_idx"])
        field = str(e["field"])
        raw   = e["value"]
        if field in INT_FIELDS:
            ...
            val = str(raw)
        edit_map.setdefault(idx, []).append((field, val))

    # Apply edits and build attacked window
    attacked = []
    tampered_indices = set(edit_map.keys())
    for i, msg in enumerate(send_window):
        if i in edit_map:
            mm = ChainMsg(**msg.__dict__)   # shallow copy
            for field, val in edit_map[i]:
                setattr(mm, field, val)
            # mac_chain deliberately NOT recomputed — attacker can't forge it
            attacked.append(mm)
        else:
            attacked.append(msg)
    ...
    trace, accepted = [], 0
    first_drop_reason = None
    rx_os_tokens, rx_meas, rx_ops = [], [], []
    rx_dt, rx_steps, rx_ctrs, rx_wins = [], [], [], []
    ...
    for idx, m in enumerate(attacked):
        ...
        was_tampered = idx in tampered_indices
        trace.append({
            "i": idx, "win": rx.window_id, "step": rx.step_idx,
            "ctr": rx.global_counter, "dt_ms": rx.dt_ms,
            "op": rx.op_code, "meas": round(rx.os_meas, 6),
            "decision": decision, "reason": reason,
            "tampered": "✏️" if was_tampered else "",
            "B_win_now": wsB.window_id,
            "B_step_expected": wsB.expected_step_idx,
            "B_ctr_expected": wsB.expected_next_counter,
        })
        rx_os_tokens.append(rx.os_token); rx_meas.append(float(rx.os_meas))
        rx_ops.append(rx.op_code);        rx_dt.append(int(rx.dt_ms))
        rx_steps.append(int(rx.step_idx)); rx_ctrs.append(int(rx.global_counter))
        rx_wins.append(int(rx.window_id))
        if ok:
            accepted += 1
        elif not forensic_continue:
            break

    return {
        "ok": True, "attack_mode": "manual",
        "attack_desc": ("Manual field edits", f"{len(edits)} edit(s) applied"),
        "session_id": grant.session_id,
        "anchor": bhex(grant.anchor_state_hash, 16),
        "policy": bhex(grant.anchor_policy_hash, 16),
        "chain_key": bhex(chain_key, 16),
        ...
        "meas_range": (round(bundle.meas_range[0], 6), round(bundle.meas_range[1], 6)),
        "op_allowlist": sorted(list(bundle.op_allowlist)),
        "accepted": accepted, "sent": len(rx_meas),
        "dropped_reason": first_drop_reason,
        "tampered_indices": sorted(tampered_indices),
        "edits_applied": edits,
        "trace": trace,
        "stream": {
            "os_token": rx_os_tokens, "meas": rx_meas, "op": rx_ops,
            "dt_ms": rx_dt, "step": rx_steps, "ctr": rx_ctrs, "win": rx_wins,
        },
    }
...
tab1, tab2, tab3, tab4 = st.tabs([
    ...
    "4) 🔴 Attack Simulation + RNN Predictor",
])
...
# ----------------------------
# Tab 4 — Attack Simulation + RNN Predictor
# ----------------------------
with tab4:
    st.markdown("## Attack Simulation Dashboard + RNN Attack Predictor")
    st.caption(
        "Runs ALL 6 attack scenarios in one pass, then trains a dedicated GRU "
        "multiclass classifier to identify the attack type from the resulting trace."
    )

    with st.sidebar:
        st.divider()
        st.subheader("Tab 4 — Predictor")
        pred_steps     = st.slider("Trace steps", 24, 96, 48, 8, key="pred_steps")
        pred_n_train   = st.slider("Train samples per class", 10, 80, 30, 5, key="pred_n")
        pred_epochs    = st.slider("Predictor epochs", 20, 200, 80, 10, key="pred_ep")
        pred_seed      = st.number_input("Seed", 0, 9999, 7, 1, key="pred_seed")
        run_pred_train = st.button("① Run simulations + Train RNN predictor", type="primary", key="pred_train_btn")
        run_pred_eval  = st.button("② Predict on all 6 attack traces", key="pred_eval_btn")

    # ---- session state init ----
    if "pred_trained" not in st.session_state:
        st.session_state.pred_trained  = False
        st.session_state.pred_p        = None
        st.session_state.pred_losses   = []
        st.session_state.pred_logs     = []
        st.session_state.pred_batch    = None   # list of {attack, meta}

    # ---- STEP 1: run simulations + train ----
    if run_pred_train:
        st.session_state.pred_trained = False
        st.session_state.pred_logs    = []
        st.session_state.pred_losses  = []

        prog4 = st.progress(0.0, text="Generating attack traces…")
        batch = run_all_attacks_batch(
            steps=pred_steps, sleep_ms=0, slack_dt=10,
            slack_meas=0.02, op_pattern="rw",
        )
        st.session_state.pred_batch = batch
        prog4.progress(0.15, text="Traces done. Training RNN predictor…")

        log_box4 = st.empty()
        def pred_log(msg):
            st.session_state.pred_logs.append(msg)
            log_box4.code("\n".join(st.session_state.pred_logs[-10:]))

        t0 = time.time()
        p_pred, losses = train_pred_model(
            n_per_class=int(pred_n_train),
            epochs=int(pred_epochs),
            lr=PRED_LR,
            seed=int(pred_seed),
            log_cb=pred_log,
        )
        st.session_state.pred_p       = p_pred
        st.session_state.pred_losses  = losses
        st.session_state.pred_trained = True
        prog4.progress(1.0, text=f"Done in {time.time()-t0:.1f}s")
        st.success(f"RNN predictor trained — final loss: {losses[-1]:.4f}")

    # ---- show simulation results table ----
    if st.session_state.pred_batch:
        st.divider()
        st.markdown("### Simulation results (all 6 scenarios)")
        sim_rows = []
        for item in st.session_state.pred_batch:
            m = item["meta"]
            sim_rows.append({
                "attack":        item["attack"],
                "accepted":      m.get("accepted", 0),
                "collected":     m.get("sent", 0),
                "first_drop":    m.get("dropped_reason") or "—",
                "dt_range":      str(m.get("dt_range", "")),
                "meas_range":    str(m.get("meas_range", "")),
            })
        st.dataframe(sim_rows, use_container_width=True, height=260)

        # --- per-attack meas timeline ---
        st.markdown("### os_meas timelines per attack")
        cols_atk = st.columns(3)
        for ci, item in enumerate(st.session_state.pred_batch):
            trace = item["meta"].get("trace", [])
            if trace:
                with cols_atk[ci % 3]:
                    st.markdown(f"**{item['attack']}**")
                    st.line_chart([r["meas"] for r in trace], height=120)

    # ---- show training loss ----
    if st.session_state.pred_losses:
        st.divider()
        st.markdown("### RNN predictor training loss")
        st.line_chart(st.session_state.pred_losses)

    # ---- STEP 2: predict on each attack trace ----
    if run_pred_eval:
        if not st.session_state.pred_trained:
            st.warning("Train the predictor first (button ①).")
        elif not st.session_state.pred_batch:
            st.warning("No batch traces found — run ① first.")
        else:
            st.divider()
            st.markdown("### RNN Prediction results")
            pred_rows = []
            for item in st.session_state.pred_batch:
                trace = item["meta"].get("trace", [])
                if not trace:
                    continue
                res = predict_attack(st.session_state.pred_p, trace)
                correct = (res["pred_label"] == item["attack"])
                pred_rows.append({
                    "true_attack":  item["attack"],
                    "predicted":    res["pred_label"],
                    "confidence":   f"{res['confidence']:.3f}",
                    "correct":      "✅" if correct else "❌",
                    **{f"p({lbl})": f"{res['probs'][lbl]:.3f}" for lbl in ATK_LABELS},
                })
            st.dataframe(pred_rows, use_container_width=True, height=280)

            # ---- probability heatmap (manual) ----
            st.markdown("### Probability heatmap (true attack × predicted class)")
            mat = np.zeros((ATK_N, ATK_N), dtype=np.float32)
            for row in pred_rows:
                ti = ATK_LABELS.index(row["true_attack"])
                for j, lbl in enumerate(ATK_LABELS):
                    mat[ti, j] = float(row[f"p({lbl})"])
            heatmap_rows = []
            for ti, true_lbl in enumerate(ATK_LABELS):
                # Use {**a, **b} instead of a | b — compatible with Python 3.8+
                row_dict = {"true \\ pred": true_lbl}
                row_dict.update({lbl: f"{mat[ti, j]:.3f}" for j, lbl in enumerate(ATK_LABELS)})
                heatmap_rows.append(row_dict)
            st.dataframe(heatmap_rows, use_container_width=True)

            # ---- live probe: run one more trace and predict ----
            st.divider()
            st.markdown("### Live probe — pick an attack and predict")
            probe_atk = st.selectbox("Attack to probe", ATK_LABELS, key="probe_atk")
            if st.button("Run probe trace + predict", key="probe_btn"):
                probe_meta = simulate_protocol_run(
                    steps=pred_steps, attack_mode=probe_atk,
                    sleep_ms=0, slack_dt=10, slack_meas=0.02,
                    op_pattern="rw", forensic_continue=True,
                )
                probe_res = predict_attack(st.session_state.pred_p, probe_meta.get("trace", []))
                correct_probe = probe_res["pred_label"] == probe_atk
                pA, pB, pC = st.columns(3)
                with pA:
                    st.metric("True attack",  probe_atk)
                with pB:
                    st.metric("Predicted",    probe_res["pred_label"])
                with pC:
                    st.metric("Confidence",   f"{probe_res['confidence']:.3f}",
                              delta="✅ correct" if correct_probe else "❌ wrong")
                st.markdown("**Per-class probabilities**")
                st.bar_chart(probe_res["probs"])

    # =========================================================
    # MANUAL ATTACK EDITOR
    # =========================================================
    st.divider()
    st.markdown("## ✏️ Manual Attack Editor")
    st.caption(
        "Pick any message index and edit its fields directly. "
        "Add as many edits as you want before running. "
        "The MAC chain is intentionally left broken so the verifier "
        "responds to whichever gate fires first."
    )

    # ---- session state for editor ----
    if "manual_edits" not in st.session_state:
        st.session_state.manual_edits = []   # list of {msg_idx, field, value}
    ...
    EDITABLE_FIELDS = ["dt_ms", "op_code", "step_idx", "global_counter",
                       "window_id", "os_meas", "session_id"]
    FIELD_DEFAULTS  = {
        "dt_ms": "999999", "op_code": "CONTROL",
        "step_idx": "99",  "global_counter": "0",
        "window_id": "99", "os_meas": "0.999",
        "session_id": "deadbeef",
    }
    FIELD_HELP = {
    ...
    # ---- sidebar controls for manual run ----
    with st.sidebar:
        st.divider()
        st.subheader("Manual editor settings")
        man_steps    = st.slider("Steps (manual run)", 8, 96, 48, 4, key="man_steps")
        man_forensic = st.checkbox("Forensic continue", value=True, key="man_forensic")
        man_op       = st.selectbox("Op pattern", ["rw","read","write"], key="man_op")

    # ---- add an edit row ----
    st.markdown("### Add edit")
    ea, eb, ec, ed = st.columns([1, 1.6, 2, 1])
    with ea:
        new_idx   = st.number_input("Msg index", min_value=0, max_value=man_steps-1,
                                     value=0, step=1, key="new_idx")
    with eb:
        new_field = st.selectbox("Field to change", EDITABLE_FIELDS, key="new_field")
    with ec:
        new_val   = st.text_input("New value",
                                   value=FIELD_DEFAULTS.get(new_field, ""),
                                   help=FIELD_HELP.get(new_field, ""),
                                   key="new_val")
    with ed:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("➕ Add edit", key="add_edit_btn"):
    ...
    # ---- show current edit queue ----
    if st.session_state.manual_edits:
        st.markdown("### Edit queue")
        eq_cols = st.columns([3, 1])
        with eq_cols[0]:
            st.dataframe(st.session_state.manual_edits, use_container_width=True,
                         height=min(180, 44 + 35 * len(st.session_state.manual_edits)))
        with eq_cols[1]:
            st.markdown("<br>", unsafe_allow_html=True)
            remove_idx = st.number_input("Remove edit #", min_value=0,
                                          max_value=max(0, len(st.session_state.manual_edits)-1),
                                          step=1, key="remove_idx")
            if st.button("🗑 Remove", key="remove_edit_btn"):
                if 0 <= int(remove_idx) < len(st.session_state.manual_edits):
                    st.session_state.manual_edits.pop(int(remove_idx))
            if st.button("🧹 Clear all", key="clear_edits_btn"):
                st.session_state.manual_edits = []
    else:
        st.info("No edits queued yet — add at least one above.")

    # ---- run manual attack ----
    run_manual = st.button("🚀 Run manual attack", type="primary", key="run_manual_btn",
                            disabled=len(st.session_state.manual_edits) == 0)
    if run_manual:
        with st.spinner("Running manual attack simulation…"):
            st.session_state.manual_result = simulate_manual_attack(
                steps=man_steps,
                ...
                slack_meas=0.02,
                op_pattern=man_op,
                forensic_continue=man_forensic,
            )
    ...
    if st.session_state.manual_result:
        ...
        if not mr.get("ok"):
            st.error("Simulation failed: " + mr.get("reason",""))
        else:
            st.divider()
            st.markdown("### Manual attack results")
            rA, rB, rC, rD = st.columns(4)
            with rA: st.metric("Accepted",    mr["accepted"])
            with rB: st.metric("Collected",   mr["sent"])
            with rC: st.metric("Tampered msgs", len(mr["tampered_indices"]))
            with rD:
                if mr["dropped_reason"]:
                    st.error("First DROP: " + mr["dropped_reason"])
                else:
                    st.success("No drops — all messages accepted")
            ...
            # full trace with tampered column highlighted
            st.markdown("#### Step-by-step trace (✏️ = tampered message)")
            trace_mr = mr["trace"]
            trace_display = [
                {k: r[k] for k in ["i","tampered","win","step","ctr","dt_ms",
                                    "op","meas","decision","reason"]}
                for r in trace_mr
            ]
            st.dataframe(trace_display, use_container_width=True, height=420)

            # timelines
            tl1, tl2, tl3 = st.columns(3)
            with tl1:
                st.markdown("**os_meas**")
                st.line_chart([r["meas"] for r in trace_mr], height=150)
            with tl2:
                st.markdown("**dt_ms**")
                st.line_chart([r["dt_ms"] for r in trace_mr], height=150)
            with tl3:
                st.markdown("**accept=1 / drop=0**")
                st.line_chart([1 if r["decision"]=="ACCEPT" else 0
                               for r in trace_mr], height=150)

            # RNN prediction on manual trace (if model trained)
            if st.session_state.get("pred_trained") and st.session_state.get("pred_p"):
                st.divider()
                st.markdown("#### 🤖 RNN prediction on this manual trace")
                pred_man = predict_attack(st.session_state.pred_p, trace_mr)
                pm1, pm2, pm3 = st.columns(3)
                with pm1: st.metric("Predicted attack", pred_man["pred_label"])
                with pm2: st.metric("Confidence",       f"{pred_man['confidence']:.3f}")
                with pm3: st.info("Train the predictor (button ①) to improve accuracy.")
                st.bar_chart(pred_man["probs"])
            else:
                st.info("Train the RNN predictor (button ①) to classify this manual trace.")
