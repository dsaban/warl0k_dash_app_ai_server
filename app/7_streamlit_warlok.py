...
if st.session_state.t5_result and st.session_state.t5_result.get("ok"):
    ...
    # ── Ground-truth banner ───────────────────────────────────────────────────
    st.markdown("### 🏷️ Simulation attack label  *(ground truth for AI evaluation)*")
    st.caption(
        "This label is automatically inferred from the fields you edited. "
        "It represents what your attack **actually is** — the correct answer "
        "the AI should predict. It is used as the training label AND as the "
        "evaluation target to score the AI."
    )

    lbl_cols = st.columns([1, 2, 1])
    with lbl_cols[0]:
        st.metric("Your edits triggered", true_label.upper(),
                  help="Derived from the field(s) you tampered with")
    with lbl_cols[1]:
        edit_summary = ", ".join(
            f"msg[{e['msg_idx']}].{e['field']}={e['value']}"
            for e in mr.get("edits_applied", [])
        )
        st.info(
            f"**Edits:** {edit_summary}\n\n"
            f"**Inferred label:** `{true_label}` "
            f"(class index {true_idx} of {N_CLASSES})"
        )
    with lbl_cols[2]:
        run_n = st.session_state.t5_run_count
        st.metric("Traces in training set", run_n,
                  help=f"All {run_n} run(s) were labelled '{true_label}' or their respective inferred label")
