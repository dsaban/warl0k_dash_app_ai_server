...
# ─────────────────────────────────────────────────────────────────────────────
# Standalone Tab 5 — Live Manual Attack Editor + Real-time RNN Retrain + Predict
#
...
# ══════════════════════════════════════════════════════════════════════════════
# ── RNN predictor — 6-class, simulation label inferred from user edits ────────
# ══════════════════════════════════════════════════════════════════════════════
# The manual attack is NOT a 7th class.  The user's edits are mapped to one of
# the 6 known attack labels so the AI can be evaluated as correct or wrong.
#
# Mapping (first matching field wins):
#   dt_ms          → timewarp   (forged timing)
#   op_code        → splice     (injected forbidden op)
#   step_idx       → reorder    (out-of-order step)
#   global_counter → replay     (replayed / forged counter)
#   window_id      → reorder    (wrong window)
#   os_meas        → splice     (forged measurement)
#   session_id     → replay     (wrong session = hijack)
#   (no edits)     → none
# ══════════════════════════════════════════════════════════════════════════════

ATK_LABELS  = ["none","reorder","drop","replay","timewarp","splice"]
N_CLASSES   = len(ATK_LABELS)   # 6
RNN_HDIM    = 64
RNN_IN_DIM  = 9
RNN_SEQ     = 48
RNN_LR      = 0.008
RNN_EPOCHS  = 50
RNN_BATCH   = 24
N_PER_STD   = 30   # synthetic samples per attack class

FIELD_TO_ATK: Dict[str,str] = {
    "dt_ms":          "timewarp",
    "op_code":        "splice",
    "step_idx":       "reorder",
...
def infer_attack_label(edits: List[Dict]) -> str:
    """Return the most appropriate attack label given the user's edit list."""
    for e in edits:
        lbl = FIELD_TO_ATK.get(str(e.get("field", "")))
        if lbl:
            return lbl
    return "none"
...
def _build_dataset(
    labelled_traces: List[Tuple[np.ndarray, int]],
    seed: int = 0,
    n_per_std: int = N_PER_STD,
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Build (X, y) for 6-class training.
    - n_per_std synthetic traces per ATK_LABELS class (baseline).
    - labelled_traces: list of (featurised_X, true_label_idx) from user runs.
      These are labelled by infer_attack_label() so the AI can be evaluated.
    """
    Xs, ys = [], []
    for ci, atk in enumerate(ATK_LABELS):
        for i in range(n_per_std):
            ...
            try:
                ...
                if not meta.get("ok") or not meta["trace"]:
                    continue
    ...
    # User simulation traces — labelled as their inferred attack class
    for X, label_idx in labelled_traces:
        Xs.append(X)
        ys.append(label_idx)
...
def _train(
    ...
    """Train a fresh 6-class RNN on std attacks + all user simulation traces."""
...
defaults = {
    ...
    "t5_labelled_traces": [],   # list of (featurised_X, label_idx) tuples
...
with st.sidebar:
    ...
    if st.button("🗑 Reset all collected traces", key="t5_reset"):
        st.session_state.t5_labelled_traces = []
...
if run_btn:
    ...
    # Infer the correct attack label from the user's edits, featurise, store
    inferred_label     = infer_attack_label(st.session_state.t5_edits)
    inferred_label_idx = ATK_LABELS.index(inferred_label)
    feat_X = _featurise(result["trace"])
    st.session_state.t5_labelled_traces.append((feat_X, inferred_label_idx))
    st.session_state.t5_run_count += 1
    # Store inferred label in result so the UI can display it
    result["inferred_label"]     = inferred_label
    result["inferred_label_idx"] = inferred_label_idx
    st.session_state.t5_result   = result   # update with label
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 3 — Results
#   TOP     : Simulation label (ground truth for AI evaluation)
#   LEFT    : Rule-based verifier verdict   (no AI)
#   RIGHT   : AI model prediction + correctness score
# ══════════════════════════════════════════════════════════════════════════════
...
if st.session_state.t5_result and st.session_state.t5_result.get("ok"):
    ...
    # Ground-truth label inferred from the user's edits
    true_label = mr.get("inferred_label", "none")
    true_idx   = mr.get("inferred_label_idx", 0)

    st.divider()
    st.markdown("## 3 · Results")

    # ── Ground-truth label block ─────────────────────────────────────────────
    st.markdown("### 🏷️ Step 1 — Confirm the attack class for AI training")
    st.markdown(
        "Before the AI can learn from your simulation, we need to tell it "
        "**what kind of attack this trace represents**. "
        "That label becomes the *correct answer* the AI is trained and scored against."
    )

    # Show each edit and why it maps to a class
    st.markdown("**What you changed → why it maps to an attack class:**")
    FIELD_REASON = {
        "dt_ms":          ("timewarp",  "Forging the timestamp (dt_ms) is the definition of a time-warp attack"),
        "op_code":        ("splice",    "Injecting a forbidden operation (op_code) is a splice / command-injection attack"),
        "step_idx":       ("reorder",   "Changing the step index breaks sequence order — that is a reorder attack"),
        "global_counter": ("replay",    "Forging the global counter mimics a replayed or duplicated message"),
        "window_id":      ("reorder",   "A wrong window ID disrupts the sequence window — consistent with reorder"),
        "os_meas":        ("splice",    "A forged os_meas value impersonates a different sensor — that is a mimic/splice"),
        "session_id":     ("replay",    "A wrong session ID is how a session-hijack / replay attack looks"),
    }

    mapping_rows = []
    for e in mr.get("edits_applied", []):
        field = e["field"]
        reason_lbl, reason_txt = FIELD_REASON.get(field, ("none", "No known attack mapping"))
        mapping_rows.append({
            "Message #":      e["msg_idx"],
            "Field edited":   field,
            "Value set to":   e["value"],
            "Maps to class":  reason_lbl.upper(),
            "Because":        reason_txt,
        })
    st.dataframe(mapping_rows, use_container_width=True,
                 height=min(220, 44 + 38 * len(mapping_rows)))

    # Label selector — auto-filled but user can override
    st.markdown(
        "**Confirm or change the attack class** — "
        "this is the label the AI will train on and be evaluated against:"
    )
    lbl_a, lbl_b = st.columns([1, 2])
    with lbl_a:
        override_label = st.selectbox(
            "Attack class (ground truth)",
            options=ATK_LABELS,
            index=ATK_LABELS.index(true_label),
            key="t5_label_override",
            help=(
                "Auto-selected from your first edited field. "
                "Change this if your edits better match a different attack class."
            ),
        )
        # If user changed the dropdown, update the stored label
        if override_label != true_label:
            true_label = override_label
            true_idx   = ATK_LABELS.index(true_label)
            mr["inferred_label"]     = true_label
            mr["inferred_label_idx"] = true_idx
            st.session_state.t5_result = mr
            # Also update the last stored labelled trace to use the new label
            if st.session_state.t5_labelled_traces:
                X_last, _ = st.session_state.t5_labelled_traces[-1]
                st.session_state.t5_labelled_traces[-1] = (X_last, true_idx)
    with lbl_b:
        CLASS_DESC = {
            "none":     "Clean traffic — no attack. The verifier should ACCEPT every message.",
            "reorder":  "Messages arrive out of sequence (step or window index wrong).",
            "drop":     "A message was silently removed — the verifier sees a gap in the counter.",
            "replay":   "A message is replayed or the session/counter is forged.",
            "timewarp": "The timing field (dt_ms) was set to an abnormal value.",
            "splice":   "A forbidden op-code was injected, or the measurement was forged.",
        }
        st.info(
            f"**{true_label.upper()}** — {CLASS_DESC.get(true_label, '')}\n\n"
            f"The verifier detected: `{mr.get('dropped_reason') or 'no drops'}`"
        )

    run_n = st.session_state.t5_run_count
    st.caption(
        f"This trace is now in the AI training set as class **`{true_label}`** "
        f"({run_n} total manual trace(s) collected this session)."
    )
    ...
    with left_col:
        ...
        st.caption(
            "Deterministic cryptographic gate checks — **no AI, no learning**. "
            "Fires the first rule that is violated. Cannot name the attack type."
        )

        if mr["dropped_reason"]:
            st.error(
                f"**ATTACK DETECTED**\n\n"
                f"First rule violated: `{mr['dropped_reason']}`"
            )
        else:
        ...
        with st.expander("Gates checked by the verifier"):
            st.code(
                f"dt_ms allowed : {mr['dt_range'][0]} - {mr['dt_range'][1]} ms\n"
                f"meas allowed  : {mr['meas_range'][0]:.4f} - {mr['meas_range'][1]:.4f}\n"
        ...
        if tampered:
            st.markdown("**Your tampered messages ✏️**")
            st.dataframe(
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # RIGHT — AI Model Prediction + Evaluation
    # ══════════════════════════════════════════════════════════════════════════
    with right_col:
        st.markdown("### 🤖 AI Model — RNN Prediction & Evaluation")
        st.caption(
            "GRU trained from scratch on 6 attack classes. "
            "Your simulation trace was added to the training set "
            f"labelled as **`{true_label}`**. "
            "The AI is then evaluated: did it predict the correct label?"
        )

        if prd is None:
            st.warning("Run an attack first.")
        else:
            predicted_label = prd["label"]
            is_correct      = predicted_label == true_label
            conf_pct        = prd["conf"] * 100

            # ── Evaluation verdict ────────────────────────────────────────────
            if is_correct:
                st.success(
                    f"✅ **AI IS CORRECT**\n\nTrue label: `{true_label}` — Predicted: `{predicted_label}` — Confidence: **{conf_pct:.1f}%**"
                )
            else:
                st.error(
                    f"❌ **AI IS WRONG**\n\nTrue label: `{true_label}` — Predicted: `{predicted_label}` — Confidence: **{conf_pct:.1f}%**\n\nTry more runs to reinforce the `{true_label}` pattern, or increase training epochs in the sidebar."
                )

            # ── Score metrics ─────────────────────────────────────────────────
            m1, m2, m3, m4 = st.columns(4)
            with m1: st.metric("True label",    true_label.upper())
            with m2: st.metric("AI predicted",  predicted_label.upper(),
                               delta="✅ correct" if is_correct else "❌ wrong")
            with m3: st.metric("Confidence",    f"{conf_pct:.1f}%")
            with m4:
                # Running accuracy across all sessions this run
                history = st.session_state.get("t5_eval_history", [])
                history.append(int(is_correct))
                st.session_state.t5_eval_history = history
                acc = sum(history) / len(history) * 100
                st.metric("Session accuracy", f"{acc:.0f}%",
                          delta=f"{len(history)} run(s)")

            # ── Probability bar chart ─────────────────────────────────────────
            st.markdown(
                "**Per-class probabilities** — "
                f"the correct class (`{true_label}`) should have the tallest bar:"
            )
            bar_data = {
                f"{'→ ' if lbl == true_label else ''}{lbl}": round(prd["probs"][lbl], 4)
                for lbl in ATK_LABELS
            }
            st.bar_chart(bar_data, height=240)

            # ── Correct-class probability callout ─────────────────────────────
            correct_class_prob = prd["probs"][true_label]
            st.caption(
                f"Probability assigned to the **correct class** (`{true_label}`): "
                f"**{correct_class_prob*100:.1f}%** — "
                + ("✅ highest" if is_correct else f"❌ lower than `{predicted_label}` ({conf_pct:.1f}%)")
            )

            with st.expander("How to improve the AI", expanded=not is_correct):
                st.markdown(
                    f"- **Run more attacks** of the same type — each run adds a "
                    f"`{true_label}` example to the training set.\n"
                    "- **Increase epochs** (sidebar) so the model trains longer.\n"
                    "- **Stick to one attack type per run** for cleaner evaluation.\n"
                    "- **Reset** (sidebar) to start a fresh experiment."
                )

        # ── Training loss ─────────────────────────────────────────────────────
        if st.session_state.t5_losses:
            st.markdown("**Training loss** (decreasing = AI is learning)")
            final = st.session_state.t5_losses[-1]
            st.line_chart(st.session_state.t5_losses, height=140)
            quality = "✅ converged" if final < 0.5 else ("⚠️ partial" if final < 1.2 else "❌ still learning")
    ...
    st.markdown(
        "### Full message trace\n"
        "`decision` / `reason` = rule-based verifier output. tampered = your edited messages."
    )
    disp_cols = ["i","tampered","win","step","ctr","dt_ms","op","meas","decision","reason"]
    st.dataframe(
        [{k: r[k] for k in disp_cols} for r in trace],
...
st.divider()
history = st.session_state.get("t5_eval_history", [])
acc_str = f"{sum(history)/len(history)*100:.0f}% ({sum(history)}/{len(history)})" if history else "n/a"
st.caption(
    f"Session: {st.session_state.t5_run_count} run(s).  "
    f"AI accuracy this session: **{acc_str}**.  "
    "Each run retrains the RNN from scratch with your trace added to the correct attack class.  "
    "Reset via the sidebar to start a new experiment."
)
