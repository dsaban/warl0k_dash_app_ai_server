// Gated RNN in C++ with naive backpropagation (FIXED)
#include <iostream>
#include <vector>
#include <string>
#include <random>
...
// Full BPTT + cross-entropy update
        ...
        for (size_t t = 0; t < T; ++t) {
            for (int i = 0; i < hidden_dim; ++i) {
                ...
                for (int j = 0; j < input_dim; ++j)
                    hs[t+1][i] += Wxh[i][j] * input_seq[t][j];
                for (int j = 0; j < hidden_dim; ++j)
                    hs[t+1][i] += Whh[i][j] * hs[t][j];
...
int main() {
    ...
    for (int i = 0; i < 32; ++i) {
        target += vocab[dis(gen)];
    }

    cout << "Target: " << target << endl;

    // int seq_len = target.size();
    ...
    RNN net(input_dim, hidden_dim, input_dim);
    ...
    cout << "Final prediction: " << net.predict(x_seq, vocab) << endl;
    cout << "Target: " << target << endl;
    ...
    size_t ram_bytes = 0;
    ram_bytes += input_dim * 32 * sizeof(double);  // Wxh
    ram_bytes += 32 * 32 * sizeof(double);        // Whh
    ram_bytes += input_dim * 32 * sizeof(double); // Why
    ram_bytes += 32 * sizeof(double);             // bh
    ram_bytes += input_dim * sizeof(double);      // by
