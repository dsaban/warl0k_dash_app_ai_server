...
## Patient State Graph
The Patient State tab renders a visual patient graph (patient → pregnancy episode → facts/events → gold care checks → actions).
Nodes are colored by status (OK/DUE/OVERDUE/UNKNOWN/NOT_APPLICABLE) and each check remains evidence-locked.
