...
class DataStore:
    ...
    def get_lexicon_rows(self) -> List[Dict[str, Any]]:
        """Return lexicon as list of dict rows (entity_id, canonical_name, synonyms...)."""
        try:
            if self.lexicon is None:
                return []
            entries = getattr(self.lexicon, 'entries', None)
            if entries is None:
                return []
            out = []
            for e in entries:
                out.append({
                    ...
                    'type': getattr(e,'type',''),
                    'synonyms': '|'.join(getattr(e,'synonyms',[]) or []),
                    'codes': getattr(e,'codes','') if hasattr(e,'codes') else ''
                })
            return out
        except Exception:
            return []
