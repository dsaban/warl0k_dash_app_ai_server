...
if mode == "Evidence Search":
    ...
    with left_g:
        ...
        clicked = plotly_events(fig, click_event=True, hover_event=False, select_event=False, override_height=520, key=f"graph_{pid}")
    with right_g:
        st.markdown("### Details")
        sel = None
        if clicked and len(clicked) > 0:
            sel = clicked[0].get('customdata')
        if sel and sel in node_meta:
            ...
            if kind == 'patient':
                ...
                st.write(f"**Claim {data.get('claim_id')}**")
                ...
                st.caption(f"{data.get('doc','')} • {data.get('pid','')}")
