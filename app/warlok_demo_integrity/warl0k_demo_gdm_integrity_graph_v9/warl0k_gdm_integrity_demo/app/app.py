...
from core.graph_ui_streamlit import build_patient_graph_agraph
...
if mode == "Evidence Search":
    ...
    with left_g:
        nodes, edges, config, node_meta = build_patient_graph_agraph(patient, results)
        selected = agraph(nodes=nodes, edges=edges, config=config)
    with right_g:
        st.markdown("### Details")
        sel = None
        # streamlit-agraph returns a dict for the selected node/edge, depending on interaction
        if isinstance(selected, dict):
            sel = selected.get("id") or selected.get("label")
        if sel and sel in node_meta:
            m = node_meta[sel]
            kind = m.get("kind")
            st.write(f"**Selected:** {sel}")
            if kind == "patient":
                st.json(m.get("patient", {}))
            elif kind == "episode":
                st.json(m.get("patient", {}))
            elif kind == "fact":
                st.json(m)
                ...
                for ev in (data.get("evidence") or [])[:5]:
                    st.caption(f"{ev.get('doc','')} • {ev.get('pid','')} • score={ev.get('score',0):.3f}")
                    st.write(ev.get("text",""))
                # show claim links if present
                rcids = data.get("related_claim_ids") or []
                if rcids:
                    st.write("**Linked claims:**", ", ".join(rcids[:10]))
            elif kind == "claim":
                st.write(f"**Claim {m.get('claim_id')}**")
                st.info("Claim details are shown in the checklist below (expand relevant check), or extend claim index for richer drill-down.")
            elif kind == "evidence":
                ev = m.get("evidence", {})
                ...
                st.write("**Action**")
                st.write(m.get("recommendation",""))
            else:
                st.json(m)
        else:
            st.info("Click a node in the graph to see linked claims and evidence.")
