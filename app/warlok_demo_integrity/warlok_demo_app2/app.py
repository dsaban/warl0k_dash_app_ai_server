...
# Persistent (per-session) store of pasted free answers keyed by qid
...
with tab_qbank:
    ...
    with left:
        ...
        m_locked = core.support_metrics(claims, C, row.get("locked_answer", ""))
        st.markdown(
            f"**Integrity (locked baseline):** `{core.integrity_label(m_locked)}`  •  "
            f"support={m_locked['support_rate']:.3f}  •  contradictions={m_locked['contradictions']}"
        )
    ...
    with right:
        ...
        pasted = st.text_area(
            ...
            value=existing,
            height=220,
            placeholder="Paste the GPT answer here…"
        )
        ...
        if score_now:
            ...
            st.markdown(
                f"**Integrity (free):** `{core.integrity_label(m_free)}`  •  "
                f"support={m_free['support_rate']:.3f}  •  contradictions={m_free['contradictions']}"
            )
        ...
        if st.session_state["free_answers_map"]:
            ...
            st.download_button(
                "Download free_answers.jsonl",
                data=data,
                file_name="free_answers.jsonl",
                mime="application/jsonl"
            )
        else:
            st.caption("No saved answers yet.")

# ---------------- Tab 1: Ad-hoc question ----------------
with tab1:
    ...
    with colA:
        ...
        q = st.text_area(
            "Question",
            value="In a low-risk pregnancy, when is GDM screening usually performed and what test is typically used?",
            height=90
        )
    ...
    with colB:
        ...
        free = st.text_area(
            "Paste a free-form answer (e.g., ChatGPT)",
            value="",
            height=180,
            placeholder="Paste any answer here to score it against the claim store…"
        )
