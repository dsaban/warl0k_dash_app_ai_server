...
def build_matchers(claims, eval_rows):
    # Baseline claim matrix for hashed matcher + learned evaluator features
    C = core.build_claim_matrix(claims)
    learned = core.fit_learned_evaluator(eval_rows, claims, C)

    # SBERT-lite matcher (trained once, persisted)
    ...
    return C, learned, sbert, claim_embs

claims = load_claims()
eval_rows = load_eval()
eval_by_id = {r["qid"]: r for r in eval_rows}
C, learned_model, sbert_model, claim_embs = build_matchers(claims, eval_rows)
...
matcher = st.sidebar.radio("Evidence matcher", ["SBERT-lite (recommended)", "Hasher (baseline)"], index=0)
...
with tab1:
    ...
    with colA:
        ...
        if st.button("Generate evidence-locked answer", type="primary", key="gen_ad_hoc"):
            top, sims = (core.sbert_retrieve(claims, claim_embs, q, sbert_model, k=k) if matcher.startswith('SBERT') else core.retrieve(claims, C, q, k=k))
...
with tab2:
    ...
    report = core.batch_score(eval_rows, claims, C, free_map=free_map, learned_model=learned_model)
