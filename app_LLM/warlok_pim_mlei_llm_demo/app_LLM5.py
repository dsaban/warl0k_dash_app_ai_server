...
# -----------------------------
# Anchor-bound per-step authentication (HKDF/HMAC)
# -----------------------------
...
def derive_step_key(anchor_key_hex: str, sid: str, ctr: int, prev_hash: str, epoch: int) -> bytes:
    """
    Derive a per-step key from the session anchor (demo-grade HKDF-ish).
    Production: use HKDF and keep anchor in a protected boundary (TEE/TPM).
    """
...
def compute_envelope_tag(step_key: bytes, canonical_core_json: str) -> str:
    """Hex tag authenticating the canonical core."""
...
class PIMState:
    ...
    anchor: str = "ANCHOR0"
    epoch: int = 0  # capability/proof epoch
    verified_until_ctr: int = 0  # capability window end ctr
    window_idx: int = 0
...
def build_env(state: PIMState, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    env["h"] = h
    env["tag"] = tag
    env["canonical_core"] = canonical_core  # convenient for audit/inspection
    return env
...
def verify_env_report(state: PIMState, env: Dict[str, Any], max_skew_s: float) -> Dict[str, Any]:
    ...
    report: Dict[str, Any] = {
        "ok": True,
        "checks": {"sid": True, "ctr": True, "prev": True, "skew": True, "hash": True, "tag": True},
    ...
    # epoch must match the current capability epoch
    ...
    if not report["ok"] and report["reason"] == "OK":
...
def build_sequence_mermaid(tr: List[Dict[str, Any]], focus_step: Optional[int] = None) -> str:
    ...
    - Explicit Step 0: START handshake + PROOF_OK capability receipt
    - Steps 1..N: normal command flow (plan -> SG gate -> tunnel -> EG verify/gate/exec -> reply -> SG verify)
    """
    lines = [
        "sequenceDiagram",
    ...
    # ---------- Step 0: anchored bootstrap ----------
    anchor_ev = next((e for e in tr if e.get("event") == "SESSION_START_ANCHOR"), None)
    if anchor_ev:
        lines.append(f"Note over SG,EG: Anchor commit={anchor_ev.get('anchor_commit','')}  cap_window={anchor_ev.get('cap_window_size','?')}")

    # START envelope (SG -> EG)
    start_out = next((e for e in tr if e.get("event") in ("PIM_ENVELOPE_OUTBOUND_START",) and int(e.get("step", -1)) == 0), None)
    ...
    if start_ver:
        ...
        if env:
            lines.append(f"Note over EG: START fields ctr={env.get('ctr','?')} h={str(env.get('h',''))[:8]} tag={str(env.get('tag',''))[:8]}")
    if proof_out and isinstance(proof_out.get("env"), dict):
        ...
        lines.append(f"EG->>T: seal(PROOF_OK ctr={env.get('ctr','?')} until={args.get('verified_until_ctr','?')})")
    ...
    for step in steps:
        ...
        if cap_den:
            lines.append(f"Note over EG: CAPABILITY_DENY 🛑 {cap_den.get('reason','')}")
            continue
        ...
        if pv_sg:
            ...
            lines.append(f"Note over SG: PIM verify(reply) {icon_for_status(ok)} ({pv_sg.get('reason','')})")
        if cap_ref:
            lines.append(f"Note over SG: CAPABILITY_REFRESH ✅ until={cap_ref.get('verified_until_ctr','?')} epoch={cap_ref.get('epoch','?')}")
...
def build_heatmap(tr: List[Dict[str, Any]]) -> pd.DataFrame:
    steps = sorted({int(e["step"]) for e in tr if isinstance(e.get("step"), int) and 1 <= e["step"] < 999999})
    cols = ["pim_sid", "pim_ctr", "pim_prev", "pim_skew", "pim_hash", "pim_tag", "sg_allow", "eg_allow"]
...
# -----------------------------
# run_flow (core demo) — refactored Anchor + Capability
# -----------------------------
def run_flow(
    task_prompt: str,
    cloud_attack: Any,
    ...
    # chain states (SG = Session Gateway, EG = Execution Guard)
    sg_pim = PIMState(sid=sid)
    eg_pim = PIMState(sid=sid)

    # transcript created ONCE
    ...
    # --- Anchor bootstrap (demo) ---
    # Production: mint/attest anchor via Hub/TEE/TPM (non-exportable).
    anchor0 = secrets.token_hex(32)
    sg_pim.anchor = anchor0
    eg_pim.anchor = anchor0
    sg_pim.epoch = 0
    eg_pim.epoch = 0
    ...
    transcript.append({
        "who": ROLE_UI,
        "event": "SESSION_START_ANCHOR",
        "sid": sid,
        "step": 0,
        "anchor_commit": safe_hash_hex(anchor0)[:16],
        "cap_window_size": 3,
        "note": "Anchor commit logged for audit; anchor value not displayed.",
    })

    # --- START handshake: SG -> EG ---
    start_payload = {"type": "START", "tool": "start", "text": "session start", "args": {"anchor_commit": safe_hash_hex(anchor0)[:16]}}
    ...
    transcript.append({"who": ROLE_SESSION_GATEWAY, "event": "TUNNEL_SEAL_START", "step": 0,
                      "nonce_len": len(start_blob.get("nonce",""))//2, "ct_len": len(start_blob.get("ct",""))//2})

    start_recv = chan.open(start_blob)
    rep_start = verify_env_report(eg_pim, start_recv, max_skew_s=max_skew_s)
    transcript.append({"who": ROLE_EXECUTION_GUARD, "event": "PIM_VERIFY_EXECUTION_GUARD_START", "step": 0,
                      "ok": rep_start["ok"], "reason": rep_start["reason"], "checks": rep_start["checks"],
                      "skew_s": rep_start["skew_s"], "computed_hash": rep_start["computed_hash"],
                      "computed_tag": rep_start.get("computed_tag"), "canonical_core": rep_start["canonical_core"],
                      "got": rep_start.get("got"), "env": start_recv})
    if rep_start["ok"]:
        advance_state(eg_pim, start_recv)
        eg_pim.window_hashes.append(start_recv["h"])

        # --- Initial capability receipt (PROOF_OK) ---
        eg_pim.verified_until_ctr = eg_pim.ctr + 3
        proof_ok_payload = {"type": "PROOF_OK", "tool": "proof_ok", "text": "capability granted",
                            "args": {"epoch": eg_pim.epoch, "verified_until_ctr": eg_pim.verified_until_ctr}}
        ...
        transcript.append({"who": ROLE_EXECUTION_GUARD, "event": "TUNNEL_SEAL_REPLY_PROOF_OK", "step": 0,
                          "nonce_len": len(proof_blob.get("nonce",""))//2, "ct_len": len(proof_blob.get("ct",""))//2})

        proof_recv = chan.open(proof_blob)
        rep_proof = verify_env_report(sg_pim, proof_recv, max_skew_s=max_skew_s)
        transcript.append({"who": ROLE_SESSION_GATEWAY, "event": "PIM_VERIFY_SESSION_GATEWAY_PROOF_OK", "step": 0,
                          "ok": rep_proof["ok"], "reason": rep_proof["reason"], "checks": rep_proof["checks"],
                          "skew_s": rep_proof["skew_s"], "computed_hash": rep_proof["computed_hash"],
                          "computed_tag": rep_proof.get("computed_tag"), "canonical_core": rep_proof["canonical_core"],
                          "got": rep_proof.get("got"), "env": proof_recv})
        if rep_proof["ok"]:
    ...
    for i, tool_call in enumerate(plan, start=1):
        ...
        if sg_pim.verified_until_ctr and (sg_pim.ctr + 1) > sg_pim.verified_until_ctr:
            transcript.append({"who": ROLE_SESSION_GATEWAY, "event": "CAPABILITY_EXPIRED", "step": i,
                              "note": f"verified_until_ctr={sg_pim.verified_until_ctr} reached; re-proof required (demo blocks)."})
            continue
        ...
        # Optional tamper on envelope
        env_sent = env
        if callable(pim_attack):
            env_sent = pim_attack(env)
            transcript.append({"who": ROLE_ATTACK, "event": "PIM_TAMPER", "step": i, "tamper": pim_attack.__name__,
                              "env_before": env, "env_after": env_sent})

        blob = chan.seal(env_sent)
        transcript.append({"who": ROLE_SESSION_GATEWAY, "event": "TUNNEL_SEAL", "step": i,
                          "nonce_len": len(blob.get("nonce",""))//2, "ct_len": len(blob.get("ct",""))//2})
        ...
        if eg_pim.verified_until_ctr and eg_pim.ctr > eg_pim.verified_until_ctr:
            transcript.append({"who": ROLE_EXECUTION_GUARD, "event": "CAPABILITY_DENY", "step": i,
                              "reason": f"capability expired verified_until_ctr={eg_pim.verified_until_ctr}"})
            continue
        ...
        # Refresh capability on EG for next few commands
        eg_pim.verified_until_ctr = eg_pim.ctr + 3
        proof_receipt = {"epoch": eg_pim.epoch, "verified_until_ctr": eg_pim.verified_until_ctr}

        reply_payload = {
            "type": "RESULT" if not is_error else "ERROR",
            "tool": "result" if not is_error else "error",
            "text": "ok" if not is_error else "exec/policy error",
            "args": {"result": result, "proof_ok": proof_receipt},
        }

        # EG reply envelope
        reply_env = build_env(eg_pim, reply_payload)
        transcript.append({"who": ROLE_EXECUTION_GUARD, "event": "PIM_ENVELOPE_REPLY_OUTBOUND", "step": i, "env": reply_env, "payload_digest": _payload_digest(reply_payload)})
        advance_state(eg_pim, reply_env)
        eg_pim.window_hashes.append(reply_env["h"])

        reply_blob = chan.seal(reply_env)
        transcript.append({"who": ROLE_EXECUTION_GUARD, "event": "TUNNEL_SEAL_REPLY", "step": i,
                          "nonce_len": len(reply_blob.get("nonce",""))//2, "ct_len": len(reply_blob.get("ct",""))//2})
        ...
        if rep_sg["ok"]:
            ...
            try:
                ...
                transcript.append({"who": ROLE_SESSION_GATEWAY, "event": "CAPABILITY_REFRESH", "step": i,
                                  "verified_until_ctr": sg_pim.verified_until_ctr, "epoch": sg_pim.epoch})
            except Exception:
                pass
    ...
    transcript.append({
        ...
        "step": 0,
        "attacker_sees": {"encrypted_blob": {"nonce": "<hex>", "ct": "<hex>"}, "note": "Only ciphertext; tamper fails AEAD + PIM tag/auth."},
    })
...
with left:
    if not tr:
        ...
        if df_chain.empty:
            ...
            cols = [
                "dir","who","step","ctr","epoch","ts","dts","derived_dts","prev_prefix","h_prefix","tag_prefix","tool","payload_digest",
