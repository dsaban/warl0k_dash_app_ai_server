...
class GatedRNNBrokerHot:
    ...
    def train(self, string_in, string_out, epochs=300):
        x_seq = self.one_hot_seq(string_in)
        t_seq = [self.vocab.index(c) for c in string_out]
        ...
        for epoch in range(epochs):
            ...
            if epoch % 150 == 0:
                ...
                print(f"[RNN Epoch {epoch}] Loss: {loss:.4f} | Recon: {recon}")

    def regenerate(self, seed_input):
        return self.predict(self.one_hot_seq(seed_input))
