...
# ============================================================
# WARL0K PIM (NumPy-only) — GRU + Attention
# Fixes in this version:
# 1) Backbone input has NO claim features (no claimed_id / expected_w leakage)
# 2) Stronger window fingerprint in generator (delta=0.25, noise=0.02)
# 3) Phase1 MS loss normalized over POSITIVES (stable, meaningful)
# 4) Token scaffold: warmup-only + low weight (optional) and forced off early
# 5) Phase2: ID & WINDOW classifiers trained on POSITIVES ONLY
#    (window/id prediction should be learned from intact chains, not shuffled/truncated)
# 6) Phase2: validity head uses claim-consistency residuals:
#    p_id_claimed, p_w_expected
# 7) Strong accept rule includes p_id_claimed and p_w_expected thresholds
# ============================================================
...
HIDDEN_DIM         = 48
ATTN_DIM           = 24
MS_HID             = 32  # MLP hidden for MS head
...
LR_PHASE1          = 0.006
LR_PHASE2_BASE     = 0.03  # decays over epochs
...
TOK_ONLY_POS       = True
TOK_STOP_EPS       = 0.25
TOK_WARMUP_EPOCHS  = 60  # after this, token loss is disabled
...
def window_delta(window_global_id, t, ms_dim=MS_DIM):
    # Stronger, deterministic window fingerprint
...
def generate_os_chain(ms_vec, window_global_id, seq_len=SEQ_LEN):
    ...
    # Normalize to m, then quantize to tokens
...
def build_dataset():
    ...
    for id_true in range(N_IDENTITIES):
        ...
        for w_true in range(N_WINDOWS_PER_ID):
            ...
            Xs.append(Xp); Ms.append(Mp); Tok.append(Tp)
            y_ms.append(ms_true); y_cls.append(0)
            true_id.append(id_true); true_w.append(wrong_w)   # TRUE window is wrong_w
            claim_id.append(id_true); expect_w.append(w_true) # verifier expects w_true

            # NEG wrong-identity chain but claimed id_true
...
def init_model(input_dim):
    ...
    # Validity head uses: [ctx, claim_id_norm, expected_w_norm, p_id_claimed, p_w_expected]
...
def attention_forward_batch(hs, Mb, p):
    u = np.tanh(hs @ p["W_att"].T)      # (B,T,A)
    scores = u @ p["v_att"]             # (B,T)
    alphas = softmax_masked(scores, Mb) # (B,T)
    ctx = np.sum(hs * alphas[:, :, None], axis=1)  # (B,H)
...
def train_phase1(p, X_ALL, M_ALL, TOK_ALL, Y_MS_ALL, Y_CLS_ALL):
    ...
    for ep in range(1, EPOCHS_PHASE1 + 1):
        ...
        for s in range(0, N, BATCH_SIZE):
            ...
            # MS MSE over positives only (normalized by positives)
            diff = (ms_hat - yms) * pos_mask
            loss_ms = 0.5 * np.sum(diff * diff) / (pos_count * MS_DIM)

            # Token scaffold: warmup-only, positives-only
            loss_tok = 0.0
            ...
            # MS MLP grads (positives only)
            dms = (diff / (pos_count * MS_DIM))  # (B,MS)
...
def ce_loss_batch_masked(logits, targets, mask_01):
    # mask_01: (B,) float 0/1. Loss averaged over masked samples only.
...
def train_phase2(p, X_ALL, M_ALL, Y_CLS_ALL, TRUE_ID, TRUE_W, CLAIM_ID, EXPECT_W):
    ...
    for ep in range(1, EPOCHS_PHASE2 + 1):
        ...
        for s in range(0, N, BATCH_SIZE):
            ...
            feat_w = np.concatenate([cb, hl, hm], axis=1)  # (B,3H)
            logits_w  = feat_w @ p["W_w"].T + p["b_w"]

            # Claim-consistency residuals (vectorized gather)
            prob_id = softmax(logits_id, axis=1)  # (B,ID)
            prob_w  = softmax(logits_w, axis=1)   # (B,W)
...
def verify_chain(p, tokens, meas, claimed_id, expected_w, true_ms=None):
    ...
    prob_id = softmax1d(logits_id)
    p_id_claimed = float(prob_id[claimed_id])

    feat_w = np.concatenate([ctx1, h_last[0], h_mean[0]], axis=0)[None, :]
    logits_w = (feat_w @ p["W_w"].T + p["b_w"])[0]
    w_pred = int(np.argmax(logits_w))
    prob_w = softmax1d(logits_w)
    p_w_expected = float(prob_w[expected_w])
    ...
    ok = (p_valid >= THRESH_P_VALID and
          id_pred == claimed_id and
          w_pred == expected_w and
          p_id_claimed >= PID_MIN and
          p_w_expected >= PW_MIN)
    ...
    out = {
        ...
        "id_pred": id_pred,
        "w_pred": w_pred,
        "p_id_claimed": p_id_claimed,
        "p_w_expected": p_w_expected,
        "ms_hat": ms_hat,
    }
...
def eval_demo(p, id_eval=0, w_eval=5):
    ...
    print("p_valid:", r["p_valid"], "id_pred:", r["id_pred"], "w_pred:", r["w_pred"],
          "pid:", r["p_id_claimed"], "pw:", r["p_w_expected"], "OK:", r["ok"])
