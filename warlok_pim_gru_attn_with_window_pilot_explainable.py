...
# A deterministic watermark-like pattern that survives normalization.
PILOT_AMP          = 0.55     # increase if window still weak (0.35-0.75 reasonable)
PILOT_FREQ_MAX     = 5        # small discrete freq choices keep it MCU-friendly
...
def window_pilot(window_global_id, seq_len=SEQ_LEN):
    """
    Deterministic, zero-mean pilot keyed by window_global_id.
    Survives normalization because it's time-varying.
    """
    rng = np.random.RandomState((window_global_id * 9176 + 11) & 0xFFFFFFFF)
    freq = int(rng.randint(1, PILOT_FREQ_MAX + 1))     # 1..PILOT_FREQ_MAX
    phase = float(rng.rand() * 2.0 * np.pi)
    t = np.arange(seq_len, dtype=np.float32)
    pilot = PILOT_AMP * np.sin(2.0*np.pi*freq*t/max(1,(seq_len)) + phase).astype(np.float32)
...
if __name__ == "__main__":
    ...
    print(f"WINDOW PILOT ENABLED: amp={PILOT_AMP}  freq_max={PILOT_FREQ_MAX}  (window-bound watermark)\n")
