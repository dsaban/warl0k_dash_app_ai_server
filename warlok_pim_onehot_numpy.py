...
def cross_entropy_loss(probs, target_idx):
    ...
    probs: (V,)
    target_idx: int
    returns: loss (scalar), grad_logits (V,)
    """
    eps = 1e-12
...
# -----------------------
# RNN + Attention, ONE-HOT version (NumPy only)
# -----------------------

class OneHotRNNAttentionPIM:
    """
    WARL0K-style core with ONE-HOT bytes (no embeddings):

      - Input bytes: indices in [0..255] -> one-hot (256)
      - Encoder RNN over [one-hot || seed_norm || counter_norm]
      - Dot-product attention using (seed,counter) as query
      - Decoder RNN over [prev_one_hot || context]
      - Output: softmax over 256 bytes per step
      - Training over multiple (seed, counter) pairs
      - Roundtrip + chain monitor

    This is deterministic (argmax over logits) and converges fast because
    the mapping from one-hot -> logits is direct.
    """

    def __init__(self, vocab_size=256, hidden_dim=64, seed=42,
        ...
        # ----- Encoder RNN -----
        # input: [vocab_size + 2]  (one-hot + seed_norm + counter_norm)
        ...
        # ----- Attention -----
        # q = [seed_norm, counter_norm] @ W_q -> (hidden_dim,)
        self.W_q = rng.normal(0, 0.1, size=(2, hidden_dim))

        # ----- Decoder RNN -----
        # input: [vocab_size + hidden_dim] (prev one-hot + context)
    ...
    def _forward_single(self, seq_bytes, seed_value, counter_value, noise_std=0.0):
        ...
        for t in range(L):
            ...
            if noise_std > 0.0:
                one_hot += np.random.normal(0.0, noise_std, size=one_hot.shape)

            x_cat = np.concatenate([one_hot, sc_vec])  # (V+2,)
        ...
        for t in range(L):
            ...
            prev_one_hot = np.zeros(self.vocab_size)
            prev_one_hot[in_idx] = 1.0

            if noise_std > 0.0:
                prev_one_hot += np.random.normal(0.0, noise_std, size=prev_one_hot.shape)

            x_cat = np.concatenate([prev_one_hot, context])  # (V+H,)
    ...
    # -----------------------
    # Backward for single (seq, seed, counter)
    # -----------------------
    def _backward_single(self, cache):
        ...
        for t in reversed(range(L)):
            ...
            d_onehot = dx_cat[:self.vocab_size]   # gradient wrt one-hot (ignored)
            d_ctx_t = dx_cat[self.vocab_size:]    # gradient wrt context
        ...
        for t in reversed(range(L)):
            ...
            # dx_cat_enc[:vocab_size] is gradient wrt one-hot -> ignored
            # dx_cat_enc[vocab_size:] would be wrt sc_vec -> treat seed/counter as constant
    ...
    # -----------------------
    # Roundtrip for single (seed, counter)
    # -----------------------
    def roundtrip(self, master_secret: bytes, seed_value, counter_value):
        seq = np.frombuffer(master_secret, dtype=np.uint8).astype(np.int64)
        L = seq.shape[0]

        loss, cache = self._forward_single(seq, seed_value, counter_value, noise_std=0.0)

        # Use attention context from cache for greedy decoding
        context = cache["context"]
        a0_dec = context @ self.W_ctx + self.b_ctx
        h_dec_prev = np.tanh(a0_dec)
    ...
    def monitor_chain(self, master_secret: bytes, seed_value,
        ...
        for i in range(length):
            ...
            loss, recon_bytes, err_rate = self.roundtrip(master_secret, seed_value, counter)
...
def demo():
    ...
    model.train(
        ...
        seed_range=(1, 4),
        counter_range=(1, 8),
        epochs=300,      # increase if not converging fully
        lr=5e-1,
        noise_std=0.0,   # keep 0.0 if you want exact training first
    )
    ...
    print("\n=== ROUNDTRIP ===")
    print("Original (raw):", master_secret)
    print("Recon    (raw):", recon_bytes)
    print("Original (hex):", master_secret.hex())
    print("Recon    (hex):", recon_bytes.hex())
    print("Byte-for-byte equal?:", recon_bytes == master_secret, " err_rate=", err_rate)
